...
import com.victor.gpw.view.portfolio.PorfolioFundPanel;
import com.victor.gpw.view.portfolio.ReportPortfolioPanel;
...
public class PortfolioPanel extends javax.swing.JPanel {
     ...
     */
    public PortfolioPanel(GpwController controller, JPanel reportPanel) {
   ...
   private void loadDataToJList(){
	   ...
	   jListPortfolioFund.setModel(new AbstractListModel<String>() { 
		   List<GpwCompany> portfolioFundList=controller.getActiveGpwCompanyList();
		   @Override
   ...
   private void buttonCheckPortfolioActionPerformed(java.awt.event.ActionEvent evt) {                                                      
        new Thread(){
        	public void run(){
        		if(companiesWithOpenSignals==null || companiesWithOpenSignals.size()==0)
        			generateOpenSignals();
        		
        		List<PorfolioFundPanel> portfolioFundPanelList= preparePortfolioFundPanels();
        		if(portfolioFundPanelList==null)
        			return;
				newPortfolioPanel = new ReportPortfolioPanel(controller,companiesWithOpenSignals,  portfolioFundPanelList);
   ...
   /** listener obslugujacy przycisk pokazujacy szczegoly dotyczace portfela */
		...
		new Thread() {
			public void run() {
				if(companiesWithOpenSignals==null || companiesWithOpenSignals.size()==0)
					...
					newPortfolioPanel = new ReportPortfolioPanel(controller,companiesWithOpenSignals );
				reportPanel.removeAll();
				reportPanel.setLayout(new java.awt.GridLayout(0, 1, 10, 0));
				reportPanel.add(newPortfolioPanel);
				reportPanel.setVisible(false);
    ...
    private void generateOpenSignals(){
    	List<GpwCompany> openSignalFundList=controller.generateOpenTransactionSingals();
    	companiesWithOpenSignals=openSignalFundList;
    }
    
    private List<PorfolioFundPanel> preparePortfolioFundPanels(){
    	List<GpwCompany>[] companyTable=controller.checkBoughtCompanies();
    	...
    	for(GpwCompany company: companyTable[0]){
    		panel=new PorfolioFundPanel(company, null, controller, false, true);
    		if(odd)
    	...
    	odd=true;
    	for(GpwCompany company: companyTable[1]){
    		panel=new PorfolioFundPanel(company, null, controller, false, null);
    		if(odd)
    	...
    	odd=true;
    	for(GpwCompany company: companyTable[2]){
    		panel=new PorfolioFundPanel(company, null, controller, false, false);
    		if(odd)
